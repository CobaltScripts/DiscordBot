...
const matchesExactOrPartial = (normalizedQuery, ...values) => {
    const safeValues = values.filter((value) => Boolean(value));
...
export class Utils {
    static async findMember(guild, query) {
        ...
        const partialMember = guild.members.cache.find((member) => matchesExactOrPartial(normalizedQuery, member.displayName, member.user.username, member.user.globalName));
        if (partialMember) {
            return partialMember.id;
        }
        try {
            const searchedMembers = await guild.members.search({ query: trimmedQuery, limit: 10 });
            const exactSearchedMember = searchedMembers.find((member) => matchesExactOrPartial(normalizedQuery, member.displayName, member.user.username, member.user.globalName));
    ...
    static async findRole(guild, query) {
        ...
        for (const role of roles.values()) {
            ...
            if (matchesExactOrPartial(normalizedQuery, role.name) ||
    ...
    static async findChannel(guild, query) {
        ...
        for (const channel of channels.values()) {
            ...
            if (matchesExactOrPartial(normalizedQuery, channel.name) ||
